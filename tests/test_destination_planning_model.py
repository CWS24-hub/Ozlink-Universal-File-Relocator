...
def test_first_live_index_for_canonical_destination_path_if_finds_first_matching_node():
    def key_fn(pl):
        return str(pl.get("item_path", "") or "").strip()
    ...
    model.append_child_payloads(
        ...
        [
            ...
            {
                "base_display_label": "File: Other",
                "name": "Other.docx",
                "is_folder": False,
                "item_path": r"Site\Lib\HR\Other.docx",
                "tag": "c",
            },
        ],
    )
    path = r"Site\Lib\HR\Contractor bank.docx"
    hit = model.first_live_index_for_canonical_destination_path_if(
        path,
        lambda i, nd: (nd or {}).get("name") == "Contractor bank.docx",
    )
    assert hit is not None
    assert (hit.data(Qt.UserRole) or {}).get("tag") == "b"
    miss = model.first_live_index_for_canonical_destination_path_if(
        path,
        lambda i, nd: (nd or {}).get("name") == "nope",
    )
    assert miss is None
